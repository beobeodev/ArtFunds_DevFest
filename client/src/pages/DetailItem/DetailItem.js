...
const DetailItem = () => {
    ...
    if (accounts.length === 0) {
      ...
      if (accountAddress === item.currentOwner) {
        ...
        await ArtFundsContract.methods
          ...
          .send({ from: accountAddress, value: web3.utils.toWei(item.price.toString(), 'Ether') })
          .on('confirmation', () => {
  ...
  const loadData = async () => {
    ...
    let obj = await ArtFundsContract.methods.listAllDigitalItem(parseInt(idItem)).call()
    ...
    <div className='itemdetail_main-container'>
      <div className='itemdetail_container-item'>
        <div className='itemdetail_container-image'>
          <img src={item.imageURL} id='image' className='itemdetail_image' alt='item' />
        </div>
        <div className='itemdetail_container-information'>
          ...
          <div className='itemdetail_button-buy'>
            <form>
              <button type='submit' className='itemdetail_btn-dark' onClick={onBuyArtwork}>
                Đặt Mua
              </button>
            </form>
          </div>
